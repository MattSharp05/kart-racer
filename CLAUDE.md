...
- `pnpm test` — unit (Vitest) · `pnpm test:perf` — perf budgets (`*.perf.test.ts`), run alone so timings are stable · `pnpm test:netsoak` — Node netcode soak (`src/net/soak*.test.ts`, 10 races at `net-bad`), run apart from `pnpm test`
...
- `pnpm item-balance` — item balance simulation (MK-72, `scripts/itemBalance.lab.ts`): 50 seeded 8-AI races over every track, prints item use / hit shares / lead changes / finish spread and checks the targets; ~5 min, run alone (`RACES=12` for fewer)
- `pnpm test:sql` — leaderboard SQL tests on a throwaway Postgres (`PG*` env vars; CI's SQL job) · `pnpm track-limits` — prints the leaderboard's `track_limits` SQL from the track data (MK-48; how to apply it: `supabase/README.md`)
...
- Online (MK-46): `/?scenario=online-race-2p&net=local&role=host|client&room=<id>[&netsim=<rtt>,<jitter>,<loss%>][&laps=n]` races tabs of one browser over BroadcastChannel (no server). E2E: `openRoom(context, n)` (the test's `context` fixture, so a room never outlives its test) + `stepAll(pages, ticks)` from `tests/e2e/online.ts`, `__game.net()` for role/kart/RTT/snapshot tick; online specs (`tests/e2e/online*.spec.ts`) run only in the `desktop-chrome-online` / `desktop-webkit-online` projects, one test at a time, each its own CI job (MK-80). `&netdebug=1` shows the net debug overlay (RTT, loss, re-sim, corrections); client smoothing / remote-kart mode live in `tuning.net` (`?tune=1` → Online).
- Every sim change gets Vitest unit tests; keep the determinism test passing.
- Playwright projects: desktop-chrome, desktop-chrome-online, desktop-webkit, desktop-webkit-online, iphone-landscape, pixel-landscape, ipad. Visual baselines are generated in CI (Docker), not locally.
