...
- Supabase `records(id, track_id, engine_class, nickname, device_id, race_ms, best_lap_ms, created_at)`. Everyone can read (RLS). Inserts go only through `submit_record()` (security definer), which rejects times below the track's minimum (`track_limits`), lap/race combinations that don't add up, and more than N submissions a minute per device. Each device keeps one best row per board. Built in MK-48: `supabase/migrations/0001_leaderboard.sql` (5 submissions a minute; `device_id` is not readable; `track_limits` also holds the board's lap count, generated by `pnpm track-limits`), and `src/records/leaderboard.ts` talks to `submit_record()` / `get_board()` over PostgREST with plain `fetch` (no supabase-js needed for two RPC calls). Only races started from the menus or a room count, never scenarios. How to apply and extend it: `supabase/README.md`.
