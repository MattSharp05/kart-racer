...
/**
 * Rubber-banding (MK-15): the AI's top-speed multiplier from its gap to the player. Behind → up to
 * +8%, ahead → up to −10%, linear in the gap and capped; exactly 1 in the AI's final 200 m, before
 * the race, or without a human player. Deterministic: sim state only.
 */
export function rubberBandScale(kart: KartState, state: SimState, geometry: TrackGeometry): number {
  const cfg = tuning.ai;
  const player = state.karts[0];
  if (!state.race.rubberBand || state.phase !== 'racing' || !player || player.ai) return 1;
