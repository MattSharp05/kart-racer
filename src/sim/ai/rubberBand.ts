 ...
 * humans, the best-placed one still racing, MK-38). Behind → up to +8%, ahead → up to −10%, linear in the gap
...
export function rubberBandScale(kart: KartState, state: SimState, geometry: TrackGeometry): number {
  ...
  if (!player) return 1;
  if (kart.race.finishTick !== undefined) return 1;
...
/** The best-placed kart driven by a person (`local` or `remote`) still racing, if any. */
function leadingHuman(state: SimState): KartState | undefined {
  for (const id of state.positions) {
    const kart = state.karts[id];
    if (kart && kart.controller !== 'ai' && kart.race.finishTick === undefined) return kart;
  }
  return undefined;
}
