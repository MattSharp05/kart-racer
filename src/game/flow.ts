...
/** Name and colour in a room before a nickname is saved (a room link opened on first launch). */
const GUEST_NAME = 'Player';
const GUEST_COLOUR = '#e63946';
...
export class Flow {
  ...
  constructor(
    ...
    // Online rooms (MK-40): the room shows the player's nickname and colour (MK-42).
    this.rooms = new RoomFlow(
      ...
      () => {
        ...
        return {
          nickname: profile?.nickname ?? GUEST_NAME,
          colour: profile ? colourHex(profile.colour) : GUEST_COLOUR,
  ...
  open(launch: Launch): void {
    ...
    switch (launch.screen) {
      ...
      case 'howToPlay': {
        ...
        const toTitle = () => {
          this.showTitleScreen();
          // A room link (MK-40): straight into that room's lobby.
          if (launch.lobby) {
